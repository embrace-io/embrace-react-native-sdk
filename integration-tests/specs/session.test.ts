...
import {getSessionPayloads} from "../helpers/embrace_api";
...
describe("Sessions", () => {
  it("should be recorded as foreground", async () => {
    ...
    attributesToFind["emb-session"].attributes[
      COMMON_ATTRIBUTES_NAME.session_id
    ] = currentSessionId;
  ...
  it("should record 2 sessions as foreground - moving the first session to background - just initialized one", async () => {
    const sessionIds = [];
    const firstSessionId = await getCurrentSessionId();
    sessionIds.push(firstSessionId);

    await driver.execute("mobile: backgroundApp", {seconds: 1});

    const secondSessionId = await getCurrentSessionId();
    ...
    sessionPayloads.Spans.forEach(span => {
      ...
      spansToFind["emb-session"].attributes[COMMON_ATTRIBUTES_NAME.session_id] =
        sessionId;
