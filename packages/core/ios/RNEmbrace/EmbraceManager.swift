...
class EmbraceManager: NSObject {
  ...
  func startView(_ viewName: String, resolver resolve: @escaping RCTPromiseResolveBlock, rejecter reject: @escaping RCTPromiseRejectBlock) {
    ...
    let startTimeMS = NSNumber(value: startTimeDate.timeIntervalSince1970 * 1000)
    
    startSpan("emb-screen-view", parentSpanId: "", startTimeMs: startTimeMS, resolver: { spanId in
      guard let spanId = spanId as? String else {
      ...
      let attributeCompletion: (Bool) -> Void = { success in
        if success {
          successfulAttributes += 1
        }
        
        if successfulAttributes == totalAttributes {
          resolve(true)
        }
      }
      
      addSpanAttributeToSpan(spanId, key: "view.name", value: viewName, resolver: { result in
        attributeCompletion(result as? Bool ?? false)
      }, rejecter: reject)
      
      addSpanAttributeToSpan(spanId, key: "emb.type", value: "ux.view", resolver: { result in
        attributeCompletion(result as? Bool ?? false)
      }, rejecter: reject)
  ...
  func endView(_ spanId: String, resolver resolve: @escaping RCTPromiseResolveBlock, rejecter reject: @escaping RCTPromiseRejectBlock) {
    ...
    stopSpan(spanId, errorCodeString: "", endTimeMs: <#T##NSNumber#>, resolver: resolve, rejecter: reject)
  }
