...
describe("Uninstall Script Android", () => {
  test("Remove Android Embrace Gradle Plugin Dependency", async () => {
    jest.mock("path", () => ({
      join: () =>
        "./packages/core/scripts/__tests__/__mocks__/android/buildWithEmbrace.gradle",
    }));
    jest.mock(
      "../../../../../../package.json",
      () => ({
    ...
    const result = removeEmbraceLinkFromFile("gradlePluginDependency");
  ...
  test("Remove Android Embrace Gradle Plugin Apply", async () => {
    jest.mock("path", () => ({
      join: () =>
        "./packages/core/scripts/__tests__/__mocks__/android/appBuildWithEmbrace.gradle",
    }));
    jest.mock(
      "../../../../../../package.json",
      () => ({
    ...
    expect(buildAppGradleFile.hasLine(androidEmbraceGradlePluginApply)).toBe(
      true,
    );
  });
  ...
  test("Unlink Embrace From MainApplication.java", async () => {
    ...
    const mainApplicationAfterRemove2 = await mainApplicationPatchable("java");

    expect(
      mainApplicationAfterRemove2.contents.includes(EMBRACE_IMPORT_JAVA),
    ).toBe(true);
    expect(
      mainApplicationAfterRemove2.contents.includes(EMBRACE_INIT_JAVA),
    ).toBe(true);
  });
