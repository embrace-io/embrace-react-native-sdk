...
class ReactNativeTracerProviderModule: NSObject {
    ...
    for (key, value) in dict {
      if let key = key as? String {
        if let value = value as? String {
          ...
          // TODO missing in API
          // https://github.com/open-telemetry/opentelemetry-swift/blob/31d22dc174530d6f207a102b1ccdecb5938a1b08/Sources/OpenTelemetryApi/Common/AttributeValue.swift#L69
          //
          // attributes.updateValue(AttributeValue(value), forKey: key)
        } else {
  ...
  @objc(spanContext:)
  func spanContext(spanBridgeId: String) -> NSDictionary {
    if let span = getSpan(id: spanBridgeId) {
      return spanContextToDict(spanContext: span.context)
    } else {
      return NSDictionary()
    }
  }
  ...
  func addLinks(spanBridgeId: String, links: NSArray) -> Void {
    // TODO, not supported by OTEL Swift API?
  }
